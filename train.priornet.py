...
def test_model(model: PriorNetwork, epoch, valid_loader_id, wandb_run: wandb.wandb_sdk.wandb_run.Run, valid_id_best, valid_pent, 
...
def generate_ood(x_train: torch.Tensor, dist: float) -> torch.Tensor:
    r"""Generate OOD samples from training set
    
    Args:
        N (int): sample number of generated ood data
        x_train (torch.Tensor): training dataset
        dist (float): disturbing scale to add noise
    
    """
    x_range = (x_train[:, 0].min() - dist, x_train[:, 0].max() + dist)
    v = np.random.uniform(x_range[0], x_range[1], size=x_train.shape)
    return torch.FloatTensor(v)

def train_model(model: PriorNetwork, epoch, train_loader, optimizer, wandb_run: wandb.wandb_sdk.wandb_run.Run, max_epochs):
    # train_epoch()
    model.train()
    ...
    with tqdm(total=len(train_loader), ncols=80, colour='green') as _tqdm:
        ...
        for data, label in train_loader:
            ...
            x_ood: torch.Tensor = generate_ood(x_id, dist=5)

            x_id = x_id.float().to('cuda')
            x_ood = x_ood.float().to('cuda')
                        
            _, alpha_id, precision_id = model(x_id)
            _, alpha_ood, precision_ood = model(x_ood)
...
def main():
    ...
    parser.add_argument('--name', type=str, required=True, help='experiment name', default='priornet')
    parser.add_argument('--note', type=str, default='runs based on priornet', help='experiment name')
